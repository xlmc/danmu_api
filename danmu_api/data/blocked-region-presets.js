// BLOCKED_WORDS 中 `地区:*`（或 `地区:全部`）启用时的内置预设地区名单。
// 仅覆盖中国大陆省级行政区与地级行政区（地级市/州/盟）；名单外的新区、区县、别称
// 请用自定义条目 `地区:名称` 补充（如 `地区:雄安`）。
// 仅供后端弹幕过滤使用，不会展示在设置页或 API 响应中。
const BLOCKED_REGION_PRESET_LIST = [
  // 省级行政区（不含港澳台）
...
export const BLOCKED_REGION_PRESET_NAMES = Object.freeze([...new Set(BLOCKED_REGION_PRESET_LIST)]);
