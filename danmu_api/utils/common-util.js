...
// parseFileName 可能在 Globals.init() 之前调用，使用静态平台集合避免
// 平台标记（例如 [Baha]）被误判为字幕组。
...
function addReleaseGroup(groups, value) {
  // `[A&B]` 表示两个独立发布组；像 `VCB-Studio` 这样的名称保持连字符。
...
/**
 * Extract explicit release-group tokens without changing the search title.
 * Only leading bracket tags and `-Group`/`@Group` suffixes are considered;
 * ordinary final title words are never guessed as a group.
 */
export function extractReleaseGroups(fileName) {
  ...
  // Require an explicit SxxEyy identity for suffix parsing so a title like
  // `Spider-Man` is never shortened to `Spider`.
...
export function parseFileName(fileName) {
  ...
  if (atIndex === -1) {
    // 没有@符号，直接返回原文件名
    return releaseResult({ cleanFileName: stripLeadingReleaseGroupTags(fileName), preferredPlatform: '' });
  }
