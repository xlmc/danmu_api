...
| TITLE_MAPPING_TABLE_URL    | 【可选】远程剧名映射表地址。映射表内容托管在 GitHub 等平台由用户维护（类似共享识别词插件），程序自动拉取并生效，与 `TITLE_MAPPING_TABLE` 合并使用（key 冲突时本地优先）。支持填 GitHub 文件页链接（自动转 raw 直链）、Gist 页面链接、jsDelivr 链接或任意文本直链。内容格式与 `TITLE_MAPPING_TABLE` 相同且更宽松：支持每行一条 `原始标题->映射标题` 规则、单行分号分隔、`#` 或 `//` 注释行、全角箭头（－>）等写法       |
| TITLE_MAPPING_TABLE_URL_CACHE_MINUTES    | 【可选】远程剧名映射表刷新间隔（分钟），默认为 `60`，设置为 `0` 表示每次请求都检查更新（不建议，会拖慢请求），最大 `1440`。拉取失败时沿用上次内容并在 5 分钟后重试，不影响正常请求       |
