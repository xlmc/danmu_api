...
配置后首次使用会尝试下载；Node/Docker 会保存到 `.cache` 并在每天北京时间 05:30 后台更新，失败时保留旧缓存。Serverless 部署在冷启动时加载，也可在设置页手动更新。
...
| TITLE_MAPPING_TABLE_URL | 【可选，默认关闭】远程剧名映射表 TXT 地址。本机 `TITLE_MAPPING_TABLE` 优先；Node/Docker 首次使用后缓存到 `.cache/title-mapping-remote.txt`，每天北京时间 05:30 后台更新，Serverless 在冷启动时加载并支持设置页手动更新。推荐：`https://raw.githubusercontent.com/xlmc/danmu-mapping/main/Word/2026.txt`       |
