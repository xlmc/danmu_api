...
| TITLE_MAPPING_TABLE_URL    | 【可选，默认关闭】远程剧名映射表 TXT 地址。配置后首次启动下载到 `.cache/title-mapping-remote.txt`，匹配时使用本地缓存；每天北京时间 05:30 自动更新，失败时保留旧缓存并重试。本机 `TITLE_MAPPING_TABLE` 优先于远程表。支持 GitHub 文件页、Gist、jsDelivr 及任意 TXT 直链；内容支持每行一条 `原始标题->映射标题`、单行分号分隔、`#`/`//` 注释、全角箭头等写法       |
